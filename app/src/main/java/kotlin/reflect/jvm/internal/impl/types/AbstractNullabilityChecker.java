            ...
            while (!supertypesDeque.isEmpty()) {
                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (supertypesPolicy2 == null) {
                        ...
                        while (it.hasNext()) {
                            SimpleTypeMarker simpleTypeMarkerMo1390transformType = supertypesPolicy2.mo1390transformType(typeCheckerState, it.next());
                            if ((typeSystemContext.isClassType(simpleTypeMarkerMo1390transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo1390transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo1390transformType)) {
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo1390transformType = supertypesPolicy.mo1390transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo1390transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo1390transformType);
