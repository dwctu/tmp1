...
public class TypeUtils {
    ...
    */
    ...
    public static List<KotlinType> getImmediateSupertypes(@NotNull KotlinType kotlinType) {
        ...
        Collection<KotlinType> collectionMo1383getSupertypes = kotlinType.getConstructor().mo1383getSupertypes();
        ArrayList arrayList = new ArrayList(collectionMo1383getSupertypes.size());
        Iterator<KotlinType> it = collectionMo1383getSupertypes.iterator();
        while (it.hasNext()) {
    ...
    public static TypeParameterDescriptor getTypeParameterDescriptorOrNull(@NotNull KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo1382getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return (TypeParameterDescriptor) kotlinType.getConstructor().mo1382getDeclarationDescriptor();
        }
        return null;
    }
    ...
    public static boolean hasNullableSuperType(@NotNull KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo1382getDeclarationDescriptor() instanceof ClassDescriptor) {
            return false;
        }
    ...
    public static boolean isNullableType(@NotNull KotlinType kotlinType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator<KotlinType> it = constructor.mo1383getSupertypes().iterator();
            while (it.hasNext()) {
                if (isNullableType(it.next())) {
                    return true;
                }
    ...
    private static boolean contains(@Nullable KotlinType kotlinType, @NotNull Function1<UnwrappedType, Boolean> function1, SmartSet<KotlinType> smartSet) {
        ...
        TypeConstructor constructor = kotlinType.getConstructor();
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator<KotlinType> it = ((IntersectionTypeConstructor) constructor).mo1383getSupertypes().iterator();
            while (it.hasNext()) {
