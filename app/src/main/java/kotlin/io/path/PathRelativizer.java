...
public final class PathRelativizer {
    ...
    public final Path tryRelativeTo(@NotNull Path path, @NotNull Path base) {
        ...
        if (Intrinsics.areEqual(r, pathNormalize) || !Intrinsics.areEqual(pathNormalize, emptyPath)) {
            ...
            r = StringsKt__StringsJVMKt.endsWith$default(string, separator, false, 2, null) ? pathRelativize.getFileSystem().getPath(StringsKt___StringsKt.dropLast(string, pathRelativize.getFileSystem().getSeparator().length()), new String[0]) : pathRelativize;
        }
