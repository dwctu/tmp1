...
public final class PublicSuffixDatabase {
    ...
    private void readTheList() throws IOException {
        ...
        pd4 pd4VarD = wd4.d(new ud4(wd4.k(resourceAsStream)));
        try {
            byte[] bArr = new byte[pd4VarD.readInt()];
            pd4VarD.readFully(bArr);
            byte[] bArr2 = new byte[pd4VarD.readInt()];
            pd4VarD.readFully(bArr2);
            if (pd4VarD != null) {
                pd4VarD.close();
            }
            synchronized (this) {
