...
public class Huffman {
    ...
    public void encode(qd4 qd4Var, od4 od4Var) throws IOException {
        long j = 0;
        int i = 0;
        for (int i2 = 0; i2 < qd4Var.s(); i2++) {
            int i3 = qd4Var.i(i2) & 255;
            int i4 = CODES[i3];
            ...
            while (i >= 8) {
                i -= 8;
                od4Var.writeByte((int) (j >> i));
            }
        }
        if (i > 0) {
            od4Var.writeByte((int) ((255 >>> i) | (j << (8 - i))));
        }
    }

    public int encodedLength(qd4 qd4Var) {
        long j = 0;
        for (int i = 0; i < qd4Var.s(); i++) {
            j += CODE_LENGTHS[qd4Var.i(i) & 255];
        }
        return (int) ((j + 7) >> 3);
    }
