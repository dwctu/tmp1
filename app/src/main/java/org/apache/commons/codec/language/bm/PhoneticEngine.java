...
public class PhoneticEngine {
    ...
    public String encode(String str, Languages.LanguageSet languageSet) {
        ...
        String strTrim = str.toLowerCase(Locale.ENGLISH).replace(SignatureImpl.SEP, ' ').trim();
        if (this.nameType == NameType.GENERIC) {
            if (strTrim.length() >= 2 && strTrim.substring(0, 2).equals("d'")) {
                String strSubstring = strTrim.substring(2);
                return "(" + encode(strSubstring) + ")-(" + encode(GoogleApiAvailabilityLight.TRACKING_SOURCE_DIALOG + strSubstring) + ")";
            }
