...
public class AttrCertExample {
    ...
    public static X509Certificate createClientCert(PublicKey publicKey, PrivateKey privateKey, PublicKey publicKey2) throws Exception {
        ...
        ASN1ObjectIdentifier aSN1ObjectIdentifier3 = X509Name.L;
        hashtable.put(aSN1ObjectIdentifier3, "Melbourne");
        ASN1ObjectIdentifier aSN1ObjectIdentifier4 = X509Name.CN;
        hashtable.put(aSN1ObjectIdentifier4, "Eric H. Echidna");
        ASN1ObjectIdentifier aSN1ObjectIdentifier5 = X509Name.EmailAddress;
        hashtable.put(aSN1ObjectIdentifier5, "feedback-crypto@bouncycastle.org");
    ...
    public static void main(String[] strArr) throws Exception {
        ...
        PrivateKey privateKeyGeneratePrivate = keyFactory.generatePrivate(rSAPrivateCrtKeySpec2);
        PublicKey publicKeyGeneratePublic = keyFactory.generatePublic(rSAPublicKeySpec2);
        keyFactory.generatePrivate(rSAPrivateCrtKeySpec);
        PublicKey publicKeyGeneratePublic2 = keyFactory.generatePublic(rSAPublicKeySpec);
        X509Certificate x509CertificateCreateAcIssuerCert = createAcIssuerCert(publicKeyGeneratePublic, privateKeyGeneratePrivate);
        X509Certificate x509CertificateCreateClientCert = createClientCert(publicKeyGeneratePublic2, privateKeyGeneratePrivate, publicKeyGeneratePublic);
        ...
        x509V2AttributeCertificateGenerator.setHolder(new AttributeCertificateHolder(x509CertificateCreateClientCert));
        x509V2AttributeCertificateGenerator.setIssuer(new AttributeCertificateIssuer(x509CertificateCreateAcIssuerCert.getSubjectX500Principal()));
        ...
        X509V2AttributeCertificate x509V2AttributeCertificate = (X509V2AttributeCertificate) x509V2AttributeCertificateGenerator.generate(privateKeyGeneratePrivate, BouncyCastleProvider.PROVIDER_NAME);
        AttributeCertificateHolder holder = x509V2AttributeCertificate.getHolder();
        if (holder.match((Certificate) x509CertificateCreateClientCert)) {
        ...
        if (issuer.match((Certificate) x509CertificateCreateAcIssuerCert)) {
        ...
        try {
            x509V2AttributeCertificate.verify(publicKeyGeneratePublic, BouncyCastleProvider.PROVIDER_NAME);
        } catch (Exception e2) {
