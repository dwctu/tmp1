...
public abstract class DSTU4145PointEncoder {
    public static ECPoint decodePoint(ECCurve eCCurve, byte[] bArr) {
        ...
        BigInteger bigIntegerValueOf = BigInteger.valueOf(bArr[bArr.length - 1] & 1);
        if (!trace(eCCurve.fromBigInteger(new BigInteger(1, bArr))).equals(eCCurve.getA().toBigInteger())) {
            bArr = Arrays.clone(bArr);
            int length = bArr.length - 1;
            bArr[length] = (byte) (bArr[length] ^ 1);
        }
        ECFieldElement eCFieldElementFromBigInteger = eCCurve.fromBigInteger(new BigInteger(1, bArr));
        if (eCFieldElementFromBigInteger.isZero()) {
            eCFieldElementMultiply = (ECFieldElement.F2m) eCCurve.getB();
            for (int i = 0; i < eCCurve.getFieldSize() - 1; i++) {
                eCFieldElementMultiply = eCFieldElementMultiply.square();
            }
        } else {
            ECFieldElement eCFieldElementSolveQuadraticEquation = solveQuadraticEquation(eCCurve, eCFieldElementFromBigInteger.add(eCCurve.getA()).add(eCCurve.getB().multiply(eCFieldElementFromBigInteger.square().invert())));
            if (eCFieldElementSolveQuadraticEquation == null) {
                throw new RuntimeException("Invalid point compression");
            }
            if (!trace(eCFieldElementSolveQuadraticEquation).equals(bigIntegerValueOf)) {
        ...
        return new ECPoint.F2m(eCCurve, eCFieldElementFromBigInteger, eCFieldElementMultiply);
    }
    ...
    public static byte[] encodePoint(ECPoint eCPoint) {
        ...
        if (!affineXCoord.isZero()) {
            if (trace(eCPointNormalize.getAffineYCoord().divide(affineXCoord)).equals(ECConstants.ONE)) {
    ...
    private static BigInteger trace(ECFieldElement eCFieldElement) {
        ...
        return eCFieldElementAdd.toBigInteger();
    }
}
