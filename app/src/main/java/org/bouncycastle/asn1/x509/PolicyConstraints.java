    ...
    private PolicyConstraints(ASN1Sequence aSN1Sequence) {
        for (int i = 0; i != aSN1Sequence.size(); i++) {
            ...
            if (aSN1TaggedObject.getTagNo() == 0) {
                this.requireExplicitPolicyMapping = DERInteger.getInstance(aSN1TaggedObject, false).getValue();
            } else {
                if (aSN1TaggedObject.getTagNo() != 1) {
                    throw new IllegalArgumentException("Unknown tag encountered.");
                }
                this.inhibitPolicyMapping = DERInteger.getInstance(aSN1TaggedObject, false).getValue();
            }
        }
    }
