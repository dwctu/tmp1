...
public class NetscapeCertRequest extends ASN1Object {
    ...
    public NetscapeCertRequest(ASN1Sequence aSN1Sequence) {
        try {
            ...
            this.pubkey = KeyFactory.getInstance(algorithmId.getObjectId().getId(), BouncyCastleProvider.PROVIDER_NAME).generatePublic(x509EncodedKeySpec);
        } catch (Exception e) {
    ...
    public boolean verify(String str) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, NoSuchProviderException {
        ...
        Signature signature = Signature.getInstance(this.sigAlg.getObjectId().getId(), BouncyCastleProvider.PROVIDER_NAME);
