...
public class PKCS12Util {
    ...
    public static byte[] convertToDefiniteLength(byte[] bArr, char[] cArr, String str) throws IOException {
        ...
        try {
            ...
            Pfx pfx2 = new Pfx(contentInfo, new MacData(new DigestInfo(new AlgorithmIdentifier(macData.getMac().getAlgorithmId().getObjectId(), (ASN1Encodable) DERNull.INSTANCE), calculatePbeMac(macData.getMac().getAlgorithmId().getObjectId(), macData.getSalt(), iIntValue, cArr, ASN1OctetString.getInstance(contentInfo.getContent()).getOctets(), str)), macData.getSalt(), iIntValue));
            byteArrayOutputStream.reset();
