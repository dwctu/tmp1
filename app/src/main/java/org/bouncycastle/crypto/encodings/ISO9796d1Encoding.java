...
public class ISO9796d1Encoding implements AsymmetricBlockCipher {
    ...
    private byte[] decodeBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        byte[] bArr2 = shadows;
        bArrConvertOutputDecryptOnly[0] = (byte) (bArr2[bArrConvertOutputDecryptOnly[1] & Ascii.SI] | (bArr2[(bArrConvertOutputDecryptOnly[1] & 255) >>> 4] << 4));
        int i4 = 0;
        boolean z = false;
        int i5 = 1;
        for (int length = bArrConvertOutputDecryptOnly.length - 1; length >= bArrConvertOutputDecryptOnly.length - (i3 * 2); length -= 2) {
            byte[] bArr3 = shadows;
            int i6 = bArr3[bArrConvertOutputDecryptOnly[length] & Ascii.SI] | (bArr3[(bArrConvertOutputDecryptOnly[length] & 255) >>> 4] << 4);
            int i7 = length - 1;
