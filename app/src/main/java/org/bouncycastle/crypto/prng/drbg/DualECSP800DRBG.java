...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    */
    ...
    private BigInteger getScalarMultipleXCoord(ECPoint eCPoint, BigInteger bigInteger) {
        return eCPoint.multiply(bigInteger).normalize().getAffineXCoord().toBigInteger();
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        for (int i = 0; i < length2; i++) {
            ...
            byte[] byteArray = this._Q.multiply(bigInteger).normalize().getAffineXCoord().toBigInteger().toByteArray();
        ...
        if (length4 < bArr.length) {
            bigInteger = getScalarMultipleXCoord(this._P, bigInteger);
            byte[] byteArray2 = this._Q.multiply(bigInteger).normalize().getAffineXCoord().toBigInteger().toByteArray();
        ...
        this._s = BigIntegers.asUnsignedByteArray(this._sLength, this._P.multiply(bigInteger).normalize().getAffineXCoord().toBigInteger());
        return length;
    }
