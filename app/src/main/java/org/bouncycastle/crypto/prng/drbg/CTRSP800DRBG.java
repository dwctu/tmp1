...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private void CTR_DRBG_Reseed_algorithm(EntropySource entropySource, byte[] bArr) throws IllegalStateException, DataLengthException, IllegalArgumentException {
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) throws IllegalStateException, DataLengthException, IllegalArgumentException {
        ...
        for (int i = 0; i < bArr.length / length; i++) {
            ...
            int i2 = i * length;
            System.arraycopy(bArr3, 0, bArr, i2, bArr.length - i2 > length ? length : bArr.length - (this._V.length * i));
        }
        CTR_DRBG_Update(bArrBlock_Cipher_df, this._Key, this._V);
        this._reseedCounter++;
