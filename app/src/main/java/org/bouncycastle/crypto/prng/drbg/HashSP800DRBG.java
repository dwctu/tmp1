...
public class HashSP800DRBG implements SP80090DRBG {
    ...
    static {
        ...
        hashtable.put(MessageDigestAlgorithms.SHA_256, Integers.valueOf(440));
        hashtable.put("SHA-512/256", Integers.valueOf(440));
        hashtable.put("SHA-512/224", Integers.valueOf(440));
        hashtable.put(MessageDigestAlgorithms.SHA_384, Integers.valueOf(888));
        hashtable.put(MessageDigestAlgorithms.SHA_512, Integers.valueOf(888));
    }
    ...
    public HashSP800DRBG(Digest digest, int i, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        byte[] bArrHash_df = Utils.hash_df(this._digest, Arrays.concatenate(entropySource.getEntropy(), bArr2, bArr), this._seedLength);
        this._V = bArrHash_df;
    ...
    private byte[] hashgen(byte[] bArr, int i) {
        ...
        for (int i3 = 0; i3 <= digestSize; i3++) {
            byte[] bArrHash = hash(bArr2);
            System.arraycopy(bArrHash, 0, bArr3, bArrHash.length * i3, i2 - (bArrHash.length * i3) > bArrHash.length ? bArrHash.length : i2 - (bArrHash.length * i3));
            addTo(bArr2, ONE);
        }
        return bArr3;
    }
    ...
    public void reseed(byte[] bArr) {
        byte[] bArrHash_df = Utils.hash_df(this._digest, Arrays.concatenate(ONE, this._V, this._entropySource.getEntropy(), bArr), this._seedLength);
        this._V = bArrHash_df;
