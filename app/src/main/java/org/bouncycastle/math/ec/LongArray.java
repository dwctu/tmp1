...
public class LongArray {
    ...
    private static long interleave2_32to64(int i) {
        int[] iArr = INTERLEAVE2_TABLE;
        return ((iArr[i & 255] | (iArr[(i >>> 8) & 255] << 16)) & UnsignedInts.INT_MASK) | ((((iArr[i >>> 24] << 16) | iArr[(i >>> 16) & 255]) & UnsignedInts.INT_MASK) << 32);
    }
    ...
    private static long interleave3_13to65(int i) {
        return (INTERLEAVE5_TABLE[i & 127] & UnsignedInts.INT_MASK) | ((r0[i >>> 7] & UnsignedInts.INT_MASK) << 35);
    }

    private static long interleave3_21to63(int i) {
        return (INTERLEAVE3_TABLE[i & 127] & UnsignedInts.INT_MASK) | ((r0[i >>> 14] & UnsignedInts.INT_MASK) << 42) | ((r0[(i >>> 7) & 127] & UnsignedInts.INT_MASK) << 21);
    }

    private static long interleave4_16to64(int i) {
        return (INTERLEAVE4_TABLE[i & 255] & UnsignedInts.INT_MASK) | ((r0[i >>> 8] & UnsignedInts.INT_MASK) << 32);
    }
