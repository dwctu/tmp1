...
public class BCMcEliecePrivateKey implements CipherParameters, PrivateKey {
    ...
    public byte[] getEncoded() {
        try {
            try {
                return new PrivateKeyInfo(new AlgorithmIdentifier(getOID(), (ASN1Encodable) DERNull.INSTANCE), new McEliecePrivateKey(new ASN1ObjectIdentifier(this.oid), this.n, this.k, this.field, this.goppaPoly, this.sInv, this.p1, this.p2, this.h, this.qInv)).getEncoded();
            } catch (IOException e) {
                e.printStackTrace();
                return null;
            }
    ...
    public String toString() {
        return (((((" length of the code          : " + this.n + IOUtils.LINE_SEPARATOR_UNIX) + " dimension of the code       : " + this.k + IOUtils.LINE_SEPARATOR_UNIX) + " irreducible Goppa polynomial: " + this.goppaPoly + IOUtils.LINE_SEPARATOR_UNIX) + " (k x k)-matrix S^-1         : " + this.sInv + IOUtils.LINE_SEPARATOR_UNIX) + " permutation P1              : " + this.p1 + IOUtils.LINE_SEPARATOR_UNIX) + " permutation P2              : " + this.p2;
    }
}
