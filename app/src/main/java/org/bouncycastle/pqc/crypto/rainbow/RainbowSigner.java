            ...
            try {
                ...
                for (int i2 = 0; i2 < length; i2++) {
                    ...
                    for (int i3 = 0; i3 < layers[i2].getOi(); i3++) {
                        sArr[i3] = sArrInitSign[i];
                        i++;
                    }
                    short[] sArrSolveEquation = this.cf.solveEquation(layers[i2].plugInVinegars(this.x), sArr);
                    if (sArrSolveEquation == null) {
                        throw new Exception("LES is not solveable!");
                    }
                    for (int i4 = 0; i4 < sArrSolveEquation.length; i4++) {
                        this.x[layers[i2].getVi() + i4] = sArrSolveEquation[i4];
                    }
                }
