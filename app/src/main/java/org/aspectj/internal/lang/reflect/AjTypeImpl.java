...
public class AjTypeImpl<T> implements AjType<T> {
    ...
    private Pointcut asPointcut(Method method) {
        ...
        if (name.startsWith(ajcMagic) && (iIndexOf = (name = name.substring(name.indexOf(PatternHead.P_M_DEF) + 2, name.length())).indexOf("$")) != -1) {
    ...
    @Override // org.aspectj.lang.reflect.AjType
    public DeclareParents[] getDeclareParents() throws SecurityException {
