...
public class GifAnimationMetaData implements Serializable, Parcelable {
    ...
    public long getDrawableAllocationByteCount(@Nullable GifDrawable gifDrawable, @IntRange(from = 1, to = WebSocketProtocol.PAYLOAD_SHORT_MAX) int i) {
        if (i >= 1 && i <= 65535) {
            return (this.mPixelsBytesCount / (i * i)) + ((gifDrawable == null || gifDrawable.mBuffer.isRecycled()) ? ((this.mWidth * this.mHeight) * 4) / r6 : Build.VERSION.SDK_INT >= 19 ? gifDrawable.mBuffer.getAllocationByteCount() : gifDrawable.getFrameByteCount());
        }
        throw new IllegalStateException("Sample size " + i + " out of range <1, " + CharCompanionObject.MAX_VALUE + SimpleComparison.GREATER_THAN_OPERATION);
    }
