...
public class ASMUtils {
    ...
    public static String[] lookupParameterNames(AccessibleObject accessibleObject) throws IOException {
        ...
        InputStream resourceAsStream = classLoader.getResourceAsStream(declaringClass.getName().replace(FilenameUtils.EXTENSION_SEPARATOR, com.broadcom.bt.util.io.IOUtils.DIR_SEPARATOR_UNIX) + ".class");
        try {
    ...
    public static String type(Class<?> cls) {
        if (!cls.isArray()) {
            return !cls.isPrimitive() ? cls.getName().replace(FilenameUtils.EXTENSION_SEPARATOR, com.broadcom.bt.util.io.IOUtils.DIR_SEPARATOR_UNIX) : getPrimitiveLetter(cls);
        }
