...
public class ParserConfig {
    ...
    public static Field getFieldFromCache(String str, Map<String, Field> map) {
        ...
        if (field == null) {
            field = map.get(Config.IN_FIELD_SEPARATOR + str);
        }
        if (field == null) {
    ...
    public Class<?> checkAutoType(String str, Class<?> cls, int i) {
        ...
        long jCharAt = (strReplace.charAt(0) ^ (-3750763034362895579L)) * 1099511628211L;
        if (jCharAt == -5808493101479473382L) {
            throw new JSONException("autoType is not support. " + str);
        }
        String str2 = "autoType is not support. ";
        if ((strReplace.charAt(strReplace.length() - 1) ^ jCharAt) * 1099511628211L == 655701488918567152L) {
            throw new JSONException(str2 + str);
        }
        long jCharAt2 = (((((strReplace.charAt(0) ^ (-3750763034362895579L)) * 1099511628211L) ^ strReplace.charAt(1)) * 1099511628211L) ^ strReplace.charAt(2)) * 1099511628211L;
        if (this.autoTypeSupport || cls != null) {
            long jCharAt3 = jCharAt2;
            int i2 = 3;
            while (i2 < strReplace.length()) {
                int i3 = i2;
                jCharAt3 = (strReplace.charAt(i2) ^ jCharAt3) * 1099511628211L;
                if (Arrays.binarySearch(this.acceptHashCodes, jCharAt3) >= 0 && (clsLoadClass = TypeUtils.loadClass(str, this.defaultClassLoader, false)) != null) {
                    return clsLoadClass;
                }
        ...
        if (!this.autoTypeSupport) {
            ...
            for (int i4 = 3; i4 < strReplace.length(); i4++) {
                jCharAt4 = (jCharAt4 ^ strReplace.charAt(i4)) * 1099511628211L;
                ...
                if (Arrays.binarySearch(this.acceptHashCodes, jCharAt4) >= 0) {
                    if (clsLoadClass == null) {
                        clsLoadClass = TypeUtils.loadClass(str, this.defaultClassLoader, false);
                    }
        ...
        if (clsLoadClass != null) {
            if (TypeUtils.getAnnotation(clsLoadClass, JSONType.class) != null) {
                return clsLoadClass;
            }
            if (ClassLoader.class.isAssignableFrom(clsLoadClass) || DataSource.class.isAssignableFrom(clsLoadClass)) {
                throw new JSONException(str3 + str);
            }
            if (cls != null) {
            ...
            if (JavaBeanInfo.build(clsLoadClass, clsLoadClass, this.propertyNamingStrategy).creatorConstructor != null && this.autoTypeSupport) {
                throw new JSONException(str3 + str);
            }
        }
        int i5 = Feature.SupportAutoType.mask;
        if ((!this.autoTypeSupport && (i & i5) == 0 && (i5 & JSON.DEFAULT_PARSER_FEATURE) == 0) ? false : true) {
            return clsLoadClass;
        }
        throw new JSONException(str3 + str);
    }
