...
public class AwtCodec implements ObjectSerializer, ObjectDeserializer {
    ...
    public Color parseColor(DefaultJSONParser defaultJSONParser) {
        ...
        while (jSONLexer.token() != 13) {
            ...
            jSONLexer.nextToken();
            if (strStringVal.equalsIgnoreCase(StreamManagement.AckRequest.ELEMENT)) {
                i = iIntValue;
        ...
        while (jSONLexer.token() != 13) {
            ...
            if (JSON.DEFAULT_TYPE_KEY.equals(strStringVal)) {
                ...
                if (strStringVal.equalsIgnoreCase("x")) {
                    ...
                    if (!strStringVal.equalsIgnoreCase(FingImageLayout.ObjectAnimatorY)) {
                        throw new JSONException("syntax error, " + strStringVal);
                    }
                    i2 = iFloatValue;
                }
    ...
    public Rectangle parseRectangle(DefaultJSONParser defaultJSONParser) {
        ...
        while (jSONLexer.token() != 13) {
            ...
            if (strStringVal.equalsIgnoreCase("x")) {
                i = iFloatValue;
            } else if (strStringVal.equalsIgnoreCase(FingImageLayout.ObjectAnimatorY)) {
                i2 = iFloatValue;
            } else if (strStringVal.equalsIgnoreCase(VideoCaptureFormat.keyWidth)) {
                i3 = iFloatValue;
            } else {
                if (!strStringVal.equalsIgnoreCase(VideoCaptureFormat.keyHeight)) {
                    throw new JSONException("syntax error, " + strStringVal);
                }
                i4 = iFloatValue;
            }
    ...
    public void write(JSONSerializer jSONSerializer, Object obj, Object obj2, Type type, int i) throws IOException {
        ...
        if (obj instanceof Point) {
            ...
            serializeWriter.writeFieldValue(writeClassName(serializeWriter, Point.class, MessageFormatter.DELIM_START), "x", point.x);
            serializeWriter.writeFieldValue(',', FingImageLayout.ObjectAnimatorY, point.y);
        } else if (obj instanceof Font) {
            Font font = (Font) obj;
            serializeWriter.writeFieldValue(writeClassName(serializeWriter, Font.class, MessageFormatter.DELIM_START), "name", font.getName());
            serializeWriter.writeFieldValue(',', "style", font.getStyle());
            serializeWriter.writeFieldValue(',', "size", font.getSize());
        } else if (obj instanceof Rectangle) {
            Rectangle rectangle = (Rectangle) obj;
            serializeWriter.writeFieldValue(writeClassName(serializeWriter, Rectangle.class, MessageFormatter.DELIM_START), "x", rectangle.x);
            serializeWriter.writeFieldValue(',', FingImageLayout.ObjectAnimatorY, rectangle.y);
            serializeWriter.writeFieldValue(',', VideoCaptureFormat.keyWidth, rectangle.width);
            serializeWriter.writeFieldValue(',', VideoCaptureFormat.keyHeight, rectangle.height);
        } else {
            if (!(obj instanceof Color)) {
                throw new JSONException("not support awt class : " + obj.getClass().getName());
            }
            Color color = (Color) obj;
            serializeWriter.writeFieldValue(writeClassName(serializeWriter, Color.class, MessageFormatter.DELIM_START), StreamManagement.AckRequest.ELEMENT, color.getRed());
