...
public class ArraySortedMap<K, V> extends ImmutableSortedMap<K, V> {
    ...
    public Iterator<Map.Entry<K, V>> reverseIteratorFrom(K k) {
        int iFindKeyOrInsertPosition = findKeyOrInsertPosition(k);
        K[] kArr = this.keys;
        return (iFindKeyOrInsertPosition >= kArr.length || this.comparator.compare(kArr[iFindKeyOrInsertPosition], k) != 0) ? iterator(iFindKeyOrInsertPosition - 1, true) : iterator(iFindKeyOrInsertPosition, true);
    }
