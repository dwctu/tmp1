...
public final class FarmHashFingerprint64 extends AbstractNonStreamingHashFunction {
    ...
    private static long hashLength0to16(byte[] bArr, int i, int i2) {
        ...
        if (i2 >= 4) {
            return hashLength16(i2 + ((LittleEndianByteArray.load32(bArr, i) & UnsignedInts.INT_MASK) << 3), LittleEndianByteArray.load32(bArr, (i + i2) - 4) & UnsignedInts.INT_MASK, (i2 * 2) + K2);
        }
        if (i2 <= 0) {
            return K2;
        }
    ...
    private static long hashLength65Plus(byte[] bArr, int i, int i2) {
        ...
        while (true) {
            ...
            if (i7 == i4) {
                ...
                long jRotateRight4 = Long.rotateRight(jRotateRight3 + jLoad642 + jArr[0] + LittleEndianByteArray.load64(bArr, i6 + 8), 37) * j3;
                long jRotateRight5 = Long.rotateRight(jLoad642 + jArr[1] + LittleEndianByteArray.load64(bArr, i6 + 48), 42) * j3;
                ...
                weakHashLength32WithSeeds(bArr, i6 + 32, jRotateRight6 + jArr2[1], LittleEndianByteArray.load64(bArr, i6 + 16) + jLoad643, jArr2);
                return hashLength16(hashLength16(jArr[0], jArr2[0], j3) + (shiftMix(jLoad643) * K0) + j4, hashLength16(jArr[1], jArr2[1], j3) + jRotateRight6, j3);
            }
            jShiftMix = j2;
            j = jLoad642;
