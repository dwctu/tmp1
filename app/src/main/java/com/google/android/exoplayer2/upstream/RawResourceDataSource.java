...
public final class RawResourceDataSource extends BaseDataSource {
    ...
    public long open(DataSpec dataSpec) throws DataSourceException, Resources.NotFoundException, NumberFormatException {
        ...
        if (TextUtils.equals(RAW_RESOURCE_SCHEME, uri.getScheme()) || (TextUtils.equals("android.resource", uri.getScheme()) && uri.getPathSegments().size() == 1 && ((String) Assertions.checkNotNull(uri.getLastPathSegment())).matches("\\d+"))) {
            ...
            String strValueOf = String.valueOf(TextUtils.isEmpty(host) ? "" : String.valueOf(host).concat(SignatureImpl.INNER_SEP));
            String strValueOf2 = String.valueOf(strSubstring);
            identifier = this.resources.getIdentifier(strValueOf2.length() != 0 ? strValueOf.concat(strValueOf2) : new String(strValueOf), OrmLiteConfigUtil.RAW_DIR_NAME, this.packageName);
            if (identifier == 0) {
        ...
        try {
            ...
            if (assetFileDescriptorOpenRawResourceFd == null) {
                ...
                StringBuilder sb = new StringBuilder(String.valueOf(strValueOf3).length() + 24);
