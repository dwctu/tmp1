...
public class MessageHideDao extends BaseDao<MessageHide> {
    ...
    public void deleteAllHides(String str, String str2) {
        try {
            this.dao.executeRawNoArgs("delete from tb_msg_hide where \"ownerJid\"=" + nd3.p(str) + " and \"friendJid\"=" + nd3.p(str2));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public List<MessageHide> getFriendHides(String str) {
        try {
            return this.dao.queryBuilder().where().eq("friendJid", nd3.p(str)).query();
        } catch (SQLException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public HashMap<String, String> getHidesToMap(String str, String str2) {
        ...
        if (hides != null && hides.size() > 0) {
            for (MessageHide messageHide : hides) {
                map.put(messageHide.getId(), messageHide.getId());
            }
        }
        return map;
    }

    public List<MessageHide> getOwnerHides(String str) {
        try {
            return this.dao.queryBuilder().where().eq("ownerJid", nd3.p(str)).query();
        } catch (SQLException e) {
            e.printStackTrace();
            return null;
        }
