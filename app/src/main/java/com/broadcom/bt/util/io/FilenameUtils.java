...
public class FilenameUtils {
    ...
    private static String doGetFullPath(String str, boolean z) {
        ...
        return iIndexOfLastSeparator < 0 ? str.substring(0, prefixLength) : str.substring(0, iIndexOfLastSeparator + (z ? 1 : 0));
    }
    ...
    private static String doGetPath(String str, int i) {
        ...
        int iIndexOfLastSeparator = indexOfLastSeparator(str);
        return (prefixLength >= str.length() || iIndexOfLastSeparator < 0) ? "" : str.substring(prefixLength, iIndexOfLastSeparator + i);
    }
            ...
            do {
                ...
                while (i < strArrSplitOnTokens.length) {
                    if (strArrSplitOnTokens[i].equals("?")) {
                        ...
                        if (z) {
                            length = strConvertCase.indexOf(strArrSplitOnTokens[i], length);
                            if (length == -1) {
                                break;
                            }
                            int iIndexOf = strConvertCase.indexOf(strArrSplitOnTokens[i], length + 1);
                            if (iIndexOf >= 0) {
                                stack.push(new int[]{i, iIndexOf});
                            }
                            length += strArrSplitOnTokens[i].length();
                        } else {
                            if (!strConvertCase.startsWith(strArrSplitOnTokens[i], length)) {
                                break;
                            }
                            length += strArrSplitOnTokens[i].length();
                        }
                ...
                if (i == strArrSplitOnTokens.length && length == strConvertCase.length()) {
                    return true;
                }
            } while (stack.size() > 0);
        }
