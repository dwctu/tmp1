...
public class AudioTrackInstantiation implements AudioController {
    ...
    private final Runnable mAudioRunnable = new Runnable() { // from class: com.spotify.sdk.android.player.AudioTrackInstantiation.1
        public final short[] pendingSamples = new short[4096];

        @Override // java.lang.Runnable
        public void run() {
            int iPeek = AudioTrackInstantiation.this.mAudioBuffer.peek(this.pendingSamples);
            if (iPeek > 0) {
                AudioTrackInstantiation.this.mAudioBuffer.remove(AudioTrackInstantiation.this.writeSamplesToAudioOutput(this.pendingSamples, iPeek));
            }
        }
    };
