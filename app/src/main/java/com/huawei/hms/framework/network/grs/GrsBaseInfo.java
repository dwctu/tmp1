...
public class GrsBaseInfo implements Cloneable {
    ...
    private StringBuffer getStringBuffer(StringBuffer stringBuffer, boolean z, Context context) throws UnsupportedEncodingException {
        ...
        String strB = new com.huawei.hms.framework.network.grs.a.c(context, "share_pre_grs_conf_").b();
        if (!TextUtils.isEmpty(strB) && !z) {
            try {
                String strEncode = URLEncoder.encode(strB, "UTF-8");
                if (!TextUtils.isEmpty(stringBuffer.toString())) {
                    stringBuffer.append(ContainerUtils.FIELD_DELIMITER);
                }
    ...
    public String getGrsParasKey(boolean z, boolean z2, Context context) {
        ...
        String strB = aVarA != null ? aVarA.b() : "";
        String appName = getAppName();
        if (!TextUtils.isEmpty(appName) && TextUtils.isEmpty(strB)) {
            stringBuffer.append("app_name");
            stringBuffer.append("=");
            stringBuffer.append(appName);
        }
        String grsReqParamJoint = getGrsReqParamJoint(z, z2, strB, context);
